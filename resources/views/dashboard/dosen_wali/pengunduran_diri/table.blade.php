<table id="dosen-second-table" class="table text-start align-middle table-bordered table-hover mb-0">
    <thead>
        <tr class="text-dark">
                    ...
                    <div class="d-flex flex-column gap-1" style="min-width: max-content;">
                        <div class="d-flex flex-nowrap gap-1">
                            <a class="btn btn-sm btn-primary"
                                href="/dashboard/dosen-wali/pengunduran-diri/{{ $pengunduran->noSurat }}">Detail</a>
                            @if ($pengunduran->status_surat != 'ditolak')
                                <a class="btn btn-sm btn-warning text-white"
                                    href="/dashboard/dosen-wali/pengunduran-diri/{{ $pengunduran->noSurat }}/edit">Ubah</a>
                            @endif
                            <form id="delete-form-{{ $pengunduran->noSurat }}"
                                action="/dashboard/dosen-wali/pengunduran-diri/{{ $pengunduran->noSurat }}" method="post"
                                class="d-inline">
                                @method('delete')
                                @csrf
                            ...
                            <a class="btn btn-sm btn-success"
                                href="/dashboard/dosen-wali/pengunduran-diri/{{ $pengunduran->noSurat }}/cetak">Cetak</a>
                        </div>
                    </div>
                </td>
                    ...
                    @if ($pengunduran->status_surat != 'selesai' && $pengunduran->status_surat != 'ditolak')
                        <div class="d-flex flex-nowrap justify-content-center gap-1" style="min-width: max-content;">
                            <button class="btn btn-sm btn-success"
                                onclick="approveDosenSuratPengunduran('{{ $pengunduran->noSurat }}')">Setujui</button>
                            <button class="btn btn-sm btn-danger"
                                onclick="rejectDosenSuratPengunduran('{{ $pengunduran->noSurat }}')">Tolak</button>
                        </div>
                    @else
...
<script>
    ...
    function approveDosenSuratPengunduran(noSurat) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: `/dashboard/dosen-wali/pengunduran-diri/${noSurat}/setujui`,
                    type: 'POST',
                    data: {
    ...
    function rejectDosenSuratPengunduran(noSurat) {
        Swal.fire({
            ...
            preConfirm: (alasan) => {
                ...
                return $.ajax({
                    url: `/dashboard/dosen-wali/pengunduran-diri/${noSurat}/tolak`,
                    type: 'POST',
                    data: {
                        _token: $('meta[name="csrf-token"]').attr('content'),
                        alasan: alasan
